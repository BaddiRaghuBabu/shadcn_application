// app/(dashboard)/connection/api-key-sage/page.tsx
"use client";
...
const SageConfigSchema = z.object({
...
export default function SageApiKeyPage() {
  const router = useRouter();
  const [connecting, setConnecting] = useState(false);

  const form = useForm<SageConfig>({
    resolver: zodResolver(SageConfigSchema),
    defaultValues: {
      applicationUrl: "",
      redirectUri: "",
      clientId: "",
  ...
  useEffect(() => {
    ...
    if (origin) {
      ...
      if (!form.getValues("redirectUri"))
        form.setValue("redirectUri", `${origin}/api/sage/callback`, { shouldDirty: true });
    }
    const load = async () => {
      try {
        NProgress.start();
        const res = await fetch("/api/sage/settings");
        if (res.ok) {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      window.location.assign("/api/sage/connect");
    } catch {
  ...
  return (
    <div className="mx-auto w-full max-w-none p-4 md:p-8">
      <style jsx global>{`
        #nprogress .bar { background: #84cc16; height: 1px; }
        #nprogress .peg { box-shadow: 0 0 10px #84cc16, 0 0 5px #84cc16; }
      `}</style>
      ...
      <Card className="border-muted">
        ...
        <CardContent className="grid gap-6">
          ...
          <div className="grid gap-1.5">
            ...
            <div className="flex items-center gap-2">
              <Input id="redirectUri" placeholder="https://yourapp.example/api/sage/callback" {...form.register("redirectUri")} disabled={connecting} />
