...
export default function QuickBooksPage() {
  ...
  useEffect(() => {
    // Auto-load both on first render
      ...
      <Card className="border-muted">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => { void loadAccounts(); void loadInvoices(); }} disabled={loadingA || loadingI}>
                {(loadingA || loadingI) ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}
                Refresh All
              </Button>
            </div>
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Kpi title="Accounts" value={formatNumberIN(totalAccounts)} hint={lastSyncA ? `Synced ${lastSyncA.toLocaleString()}` : "Not synced"} />
            <Kpi title="Total Balance" value={formatINR(totalBalance)} hint="Sum of current balances" />
            <Kpi title="Invoices" value={formatNumberIN(invCount)} hint={lastSyncI ? `Synced ${lastSyncI.toLocaleString()}` : "Not synced"} />
            <Kpi title="Open Balance" value={formatINR(invOpenAmt)} hint={`Total Amount: ${formatINR(invTotalAmt)}`} />
          </div>
          ...
          <Tabs defaultValue="accounts" className="w-full">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="secondary" className="hidden sm:inline-flex">Company: QuickBooks</Badge>
              </div>
            </div>
...
function AccountsTable({
  ...
  const types = useMemo(() => ["all", ...uniqStrings(data.map((d) => d.account_type ?? "Unknown"))], [data]);
  const subTypes = useMemo(() => ["all", ...uniqStrings(data.map((d) => d.account_sub_type ?? "Unknown"))], [data]);
    ...
    () => [
      {
        accessorKey: "name",
        header: ({ column }) => (
          <HeaderSorter label="Name" column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        cell: ({ row }) => <div className="text-right tabular-nums">{formatINR(row.original.current_balance ?? 0)}</div>,
      },
      {
        accessorKey: "account_id",
        header: "Account ID",
        cell: ({ row }) => (
          <CopyCell text={row.original.account_id} />
        ),
      },
    ],
    []
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      const v = String(row.getValue<string | null>(columnId) ?? "").toLowerCase();
  ...
  const exportCSV = () => {
    ...
    const header = ["name", "fully_qualified_name", "account_type", "account_sub_type", "current_balance", "account_id"];
    const csv = [
      header.join(","),
      ...rows.map((r) =>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex w-full items-center gap-2 md:w-auto">
          ...
          <Select value={subTypeFilter} onValueChange={setSubTypeFilter}>
            <SelectTrigger className="w-44"><SelectValue placeholder="Subtype" /></SelectTrigger>
            <SelectContent>{subTypes.map((t) => <SelectItem key={t} value={t}>{t === "all" ? "All subtypes" : t}</SelectItem>)}</SelectContent>
          </Select>
        </div>
...
function InvoicesTable({
  ...
  const currencies = useMemo(() => ["all", ...uniqStrings(data.map((d) => d.currency_code ?? "UNK"))], [data]);
  ...
  const columns = useMemo<ColumnDef<Invoice>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) =>
          <div className="tabular-nums">{formatCurrency(row.original.total_amt ?? 0, row.original.currency_code)}</div>,
      },
      {
        accessorKey: "balance",
        header: ({ column }) => <HeaderSorter label="Balance" column={column} />,
...
function HeaderSorter({ label, column }: { label: string; column: any }) {
  ...
  return (
    ...
    >
      {label}
      {sorted === "asc" ? <ArrowDownAZ className="h-4 w-4" /> : sorted === "desc" ? <ArrowUpAZ className="h-4 w-4" /> : null}
    </button>
  );
}
...
function ColumnToggle({ table }: { table: ReturnType<typeof useReactTable<any>> }) {
  return (
    <DropdownMenu>
                ...
                {hg.headers.map((h) => (
                  <TableHead key={h.id}>
                    {h.isPlaceholder
                      ...
                      ? h.column.columnDef.header({ column: h.column })
